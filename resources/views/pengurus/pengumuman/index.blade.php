...
<div class="container-fluid">
    ...
    <div class="col-sm-6">
        ...
        <li class="breadcrumb-item"><a href="{{route('pengumuman.index')}}"></a>Pengumuman</li>
        </ol> 
    </div><!-- /.col -->
    </div><!-- /.row -->
                        ...
                        @foreach ($pengumuman as $row)
                            <tr>
                                ...
                                @else
                                    <td></td>
                                @endif
                                <td><a class="mailbox-attachment-name" style="color: black" href="{{route('pengumuman.show', $row->id)}}">{{$row->judul}}</a></td>
                                <td>{{$row->updated_at}}</td>
                                <td>{{$row->users->name}}</td>
                                <td>
                                    <form action="{{route('pengumuman.destroy', $row->id)}}" method="POST">
                                        <a class="btn btn-primary btn-sm" href="{{route('pengumuman.show', $row->id)}}">
                                            <i class="fas fa-folder"></i>
                                            Lihat
                                        </a>
                                        <a class="btn btn-info btn-sm" href="{{route('pengumuman.edit', $row->id)}}">
